...
* for drawing inferences about future (or past) observable events
...
# Potential difficulties while fitting

## Difficulty 1: missing data

```{r Alien 2, error = TRUE}
Alien$pred <- Alien$resid <- Alien$density <- NULL ## Cleaning up
Alien2 <- Alien
...
## The rows with ```NA(s)``` are being omitted
...
## Difficulty 2: degenerated design matrix (trivial)
...
coef(mod_alien3_lm)
det(crossprod(model.matrix(mod_alien3_lm)))  ## when det(XTX) <= 0, XTX has no inverse!
mod_alien3_lm$rank  == ncol(model.matrix(mod_alien3_lm))
```

## Difficulty 2: degenerated design matrix (subtle)
```{r Alien4, error = TRUE}
set.seed(1L)
...
## Difficulty 2: degenerated design matrix (```n < p```)
```{r Alien5, error = TRUE}
set.seed(1L)
...
coef(mod_stats)
summary(mod_stats)$sigma^2  ## error variance
summary(mod_stats)$sigma^2 * (nrow(Alien) - length(coef_lm)) /  nrow(Alien)  ## residual variance
logLik(mod_stats)
```
...
library(spaMM)
mod_spaMM_ML <- fitme(size ~ humans_eaten, data = Alien, method = "ML")  ## default: residual variance
mod_spaMM_ML$fixef
mod_spaMM_ML$phi
...
```{r spaMM 2}
mod_spaMM_REML <- fitme(size ~ humans_eaten, data = Alien, method = "REML")  ## non-default: error variance
