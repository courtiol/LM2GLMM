...
(XTX <- crossprod(model.matrix(~ T36 + T38 + PT36 + PT38, data = FungiAlive))) ## more efficient
det(XTX)
```

The determinant close to zero shows that the matrix is not invertible.
This is because of the multicollinearity between the different levels of pre-treatments and treatments.
...
```{r pred 1}
X <- model.matrix(~ PT + T, data = FungiAlive)
(XTX <- crossprod(X))
det(XTX)
```
