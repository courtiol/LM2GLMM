---
title: "Answers to Exercises: Let's practice more LMs"
...
vignette: >
  ...
  %\VignetteIndexEntry{2.X Answers to Exercises: Let's practice more LMs}
...
```{r}
crossprod(model.matrix( ~ feed - 1, data = chickwts))
```

Removing the intercept when there is only one qualitative predictor gives the counts directly.
...
```{r plot effects, fig.height = 8, fig.width = 8}
...
par(mfrow = c(2, 2))
plot(visreg::visreg(mod_swiss_bis))
```
...
mod_trees1 <- lm(Volume ~ HG2 - 1, data = trees)
```
...
mod_trees2 <- lm(log_Volume ~ 1 + offset(log_Height + 2 * log_Girth), data = trees)
```
...
```{r r sq mod}
cor(predict(mod_trees1), trees$Volume)^2
cor(exp(predict(mod_trees2)), trees$Volume)^2
```
...
mod_trees3 <- lm(Volume ~ -1 + offset(HPG2), data = trees)
```

We can compare both models explicitly:

```{r}
anova(mod_trees1, mod_trees3)
```
...
pred1 <- predict(mod_trees1, newdata = data.for.pred)
pred2 <- predict(mod_trees3, newdata = data.for.pred)
