...
hist(stat.rand, nclass = 50, xlim = c(0, 7))
...
mod_UK_no_milk_spaMM <- fitme(height ~ sex*weight, data = mod_UK_milk_spaMM$data)
...
but only if the number of residual degrees of freedom is the same
...
You may use the AIC (AIC = -2 * (logLik - K)) (smaller is better) even if the number of residual degrees of freedom differ.
...
You may use ajusted R-squared (higher is better) even if the number of residual degrees of freedom differ, but it is not a valid statistical model comparison technic.
...
### This is the best method as it deals with uncertainty. It also gives a p-value.
...
```{r robustness 2}
set.seed(1L)
mean(replicate(200, min(Anova(lm(y ~ ., data = simu_null(N = 100, K = 1)))["Pr(>F)"][1, 1]) <= 0.05))

mean(replicate(200, min(Anova(lm(y ~ ., data = simu_null(N = 100, K = 3)))["Pr(>F)"][1:3, 1]) <= 0.05))

mean(replicate(200, min(Anova(lm(y ~ ., data = simu_null(N = 100, K = 10)))["Pr(>F)"][1:10, 1]) <= 0.05))

mean(replicate(200, min(Anova(lm(y ~ ., data = simu_null(N = 1000, K = 50)))["Pr(>F)"][1:50, 1]) <= 0.05))
```
...
abline(h = 0.05, lty = 2)
legend("topleft", fill = c("red", "green"), legend = c("step selection (backward AIC)", "full model"), bty = "n")
```
