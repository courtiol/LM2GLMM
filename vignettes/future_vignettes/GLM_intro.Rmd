...
```{r}
rbind(summary(mod_binar$fitted.values),
...
```{r}
sigma2_error <- deviance(mod_gauss) / mod_gauss$df.residual
sigma2_resid <- sigma2_error *  mod_gauss$df.residual / nrow(mod_gauss$model)
mod_gauss_sat <- spaMM::fitme(formula = size ~ -1 + offset(size), data = Aliens, fixed = list(phi = sigma2_resid))

c(mod_gauss$deviance, deviance(mod_gauss), -2*(logLik(mod_gauss) - logLik(mod_gauss_sat))*sigma2_resid)
mod_gauss_null <- update(mod_gauss, . ~ 1)
c(mod_gauss$null.deviance, mod_gauss_null$deviance)
```
...
deviance.pt <- -2 * sigma2_resid * (dnorm(x = mod_gauss$y, mean = mod_gauss$fitted.values,
                                          sd = sqrt(sigma2_resid), log = TRUE) - 
                                    dnorm(x = mod_gauss$y, mean = mod_gauss$y,
                                          sd = sqrt(sigma2_resid), log = TRUE))
(mod_gauss$prior.weight * deviance.pt)[1:2] 
c(deviance(mod_gauss), sum(mod_gauss$prior.weight * deviance.pt))
```
...
c(deviance(mod_poiss), sum(mod_poiss$prior.weight * deviance.pt))
```
...
deviance.pt  <- -2 * (dbinom(x = mod_binar$y, size = 1, prob = mod_binar$fitted.values, log = TRUE) - 
                      dbinom(x = mod_binar$y, size = 1, prob = mod_binar$y, log = TRUE))
(mod_binar$prior.weight * deviance.pt)[1:2]
c(deviance(mod_binar), sum(mod_binar$prior.weight * deviance.pt))
...
deviance.pt  <- -2 * (dbinom(x = mod_binom$y*mod_binom$prior.weights, size = mod_binom$prior.weights,
                             prob = mod_binom$fitted.values, log = TRUE) - 
                      dbinom(x = mod_binom$y*mod_binom$prior.weights, size = mod_binom$prior.weights,
                             prob = mod_binom$y, log = TRUE))
deviance.pt[1:2]  ## here the prior.weights directly considered
c(deviance(mod_binom), sum(deviance.pt))
```
...
my_glm1 <- function(formula, data, family, weights = NULL) {
  ...
  get_resid_deviance <- function(coef) {  ## define function computing the unscaled residual deviance
    ...
    return(dev)  ## return the unscaled residual deviance
  }
