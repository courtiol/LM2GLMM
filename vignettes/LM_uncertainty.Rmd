...
### The idea of bootstrap is to get a sense of the variability between datasets (non-parametric boostrap) or estimates (parametric bootstrap) that we would get if we could really draw several datasets, while considering that we cannot.
...
<font size = 5> If it was computed on an infinite number of datasets coming from the same data generating process, then a confidence interval at level 95% should contain the true (unknown) parameter value for 95% of the sets. </font>

<br>

<font size = 5> Since the observed data correspond to a randomly selected set, then it is likely that the true value of the parameter lies within the confidence interval. </font>

<br>

<font size = 5> Yet, you never know for sure (when using real data). </font>
...
# Confidence intervals: parametric bootstraps
...
## CI using proper parametric bootstrap
...
refit(data = newY(Alien, mod_stats)) ## fit on data simulated by parametric bootstrap
```
...
Note: the bias (= mean bootstraps - estimate) will be accounted for!

## CI using proper parametric bootstrap

### 3. Compute the CI using ```boot.ci```
```{r param boot proper 3}
boot.ci(sim.boot1, type = "basic")  ## try also type = "norm"

(confint_boot2 <- rbind(boot.ci(sim.boot1, type = "basic")$basic[, 4:5],
                        boot.ci(sim.boot2, type = "basic")$basic[, 4:5]))
```

## Testing the coverage of the 95% CI (proper param. boot.)
...
CIboot2b <- function(rep = 100, i){
  ...
  newYs <- simulate(mod, nsim = rep)
  res_sim <- t(coef(lm(as.matrix(newYs) ~ humans_eaten, data = data)))
  boot.ci(list(t0 = coef(mod)[[i]],
               t  = res_sim[, i, drop = FALSE], 
               R  = nrow(res_sim)), type = "basic")$basic[, 4:5]
}
set.seed(1L)
...
Note: we bypass ```boot()``` to save time (a single call to ```simulate()``` is more efficient).
...
rbind("using SE" = confint(mod_stats)[1, ], "using SE boot 1" = confint_boot[1, ],
      "using SE boot 2" = confint_boot2[1, ], "using profile" = CI_intercept(mod_stats))
```
...
rbind("using SE" = confint(mod_stats)[2, ], "using SE boot 1" = confint_boot[2, ],
      "using SE boot 2" = confint_boot2[2, ], "using profile" = CI_slope(mod_stats))
```
