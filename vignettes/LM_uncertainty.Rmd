...
* that there are many ways to get 95% CI for LM
...
(vcov.means <- X %*% vcov(mod_stats) %*% t(X))  ## covariance of predictions
diag(vcov.means)
```
...
* Cost: uses the chi-squared distribution, which is an assymptotic approximation, so it needs a lot of observations. BUT: the parametric boostrap is always a way out!
...
### Compute the 95% CI by likelihood profile without relying on the chi-squared approximation thanks to parametric bootstrap and test the coverage of this way of constructing a confidence interval.
