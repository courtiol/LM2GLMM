...
<font size = 5> The aim of the boostrap is to approximate the distribution of a statistic, in cases where parameters are not known (as usual) and where this distribution is not known analytically for given parameters. </font>
...
Note: the bias (= mean bootstrap - estimate) will be accounted for!
...
CIboot2b <- function(rep = 100, i){
  ...
  boot.ci(boot.out = list(R = nrow(res_sim)),
          t0 = coef(mod)[[i]], t  = res_sim[, i, drop = FALSE],
...
CIboot2c <- function(rep = 100, i){
  ...
  boot.ci(boot.out = list(R = nrow(res_sim)),
          t0 = coef(mod)[[i]], t  = res_sim[, i, drop = FALSE],
          var.t0 = vcov(mod)[i, i], var.t = rep(var(res_sim[, i]), nrow(res_sim)),
...
Note: here it accounts for the uncertainty in the estimation of the variance.
