...
* that boundaries are obtained using the Student distribution
* what statistical coverage is
* what the 95% CI on the mean response is
* what the difference between a confidence interval and a prediction interval is
* what likelihood profiling is
* that estimates are not necessarily independent from each others
* that there are three ways to get 95% CI for LM
...
## CI for $\widehat{\beta}$ with known variance

<font size = 6> $\widehat{\beta}\sim \mathcal{N}(\beta, \sigma_{\beta})$ </font>
...
## CI for $\widehat{\beta}$ with estimated variance

<font size = 6> $\widehat{\beta}\sim \mathcal{N}(\beta, \sigma_{\beta})$ </font>

<font size = 6> $t = \frac{\widehat{\beta} - \beta}{\widehat{\sigma_{\widehat{\beta}}}}\sim \mathcal{t}_{N - K}$ </font>
...
<font size = 6> $[\widehat{\beta} + \mathrm{Q}0.025_{\mathcal{t}_{N - K}} \times \widehat{\sigma_{\widehat{\beta}}}; \ \widehat{\beta} + \mathrm{Q}0.025_{\mathcal{t}_{N - K}} \times \widehat{\sigma_{\widehat{\beta}}}]$ </font>
...
It is still assymptotic but SE are no longer computed analytically:
