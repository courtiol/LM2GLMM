...
# Fiting more problematic models
...
Think:
```data``` $\rightarrow$ ```model.frame()``` $\rightarrow$ ```model.matrix()``` $\rightarrow$ ```qr()``` $\rightarrow$ ```qr.fitted()```
...
## Difficulty 2: model matrix not with full rank
...
coef(mod_alien3_lm)
det(crossprod(model.matrix(mod_alien3_lm)))  ## when det(XTX) == 0, the matrix is singular / degenerated / not of full rank
mod_alien3_lm$rank
```
