...
The plot shows the influence of height on the mean volume for 4 specific girths (```r round(girth.for.pred, 2)```). Thicker lines represent trees with larger girth. The black lines depict the predictions from the model ```mod_trees``` and the red ones are for the model ```mod_trees_cylinder```. The raw data are represented by the brown and yellow dots. The diameter of these dots is proportional to the actual measured girth.
