...
```{r}
anova(mod_chick)  ## here anova() is sufficient because type I and II are the same with a single predictor
```
...
```{r}
nortest::lillie.test(residuals(mod_insect_bc))
shapiro.test(residuals(mod_insect_bc))
```
Again, no issue here.
