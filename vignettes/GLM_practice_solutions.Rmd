...
pred_probit$fit <- fit_binom_probit$family$linkinv(pred_probit_res$fit)
pred_probit$lwr <- fit_binom_probit$family$linkinv(
  pred_probit_res$fit + qnorm(0.025) * pred_probit_res$se.fit)
pred_probit$upr <- fit_binom_probit$family$linkinv(
...
pred_logit$fit <- fit_binom_logit$family$linkinv(pred_logit_res$fit)
pred_logit$lwr <- fit_binom_logit$family$linkinv(
  pred_logit_res$fit + qnorm(0.025) * pred_logit_res$se.fit)
pred_logit$upr <- fit_binom_logit$family$linkinv(
...
ggplot() +
  ...
  geom_ribbon(aes(ymin = 1 - upr, ymax = 1 - lwr, x = temp, fill = link), alpha = 0.1, data = all_preds) +
...
- the uncertainty is large, but both model agree that launching the shuttle by 31 degree F was a very risky...
