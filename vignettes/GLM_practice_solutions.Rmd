...
plot(insect_glm_resid)
testTemporalAutocorrelation(insect_glm_resid, time = order(fitted(fit_insect_glm)))
```
...
## Re-testing assumptions on the negative binomial model
...
```{r, message = FALSE}
library(multcomp)
posthoc <- glht(fit_esoph_probit2, linfct = c("`drink_smoke120+_0-9g/day` - `drink_smoke0-39g/day_30+` == 0"))
par(oma = c(1, 5, 1, 1))
plot(posthoc)
confint(posthoc)
