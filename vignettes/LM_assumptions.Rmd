...
Note: the model here considers the relationship between the predictors (the response variable of ```mod_US``` was ```rape```).
...
## Many ways to be an outlier

```{r influence.measures 0}
influence.measures(mod_davis)  ## In the next slides we will see all that in details!
```
...
Both the DFFITS measure and the Cook distance (aka Cook's D) are two relative measures of the extent to which the predicted y-values changes if a given observation is dropped. DFFITS measures it scaled by the standard deviation of the fit at the point, Cook's D in the value of a F statistics comparing simultaneously the changes in all estimates when the observation is dropped or not.
