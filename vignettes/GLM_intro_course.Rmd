...
- for the scale of the **linear predictor**, e.g.:
...
newdata1 <- expand.grid(sex = levels(fit_titanic$model$sex),
                        passengerClass = levels(fit_titanic$model$passengerClass),
                        age = mean(fit_titanic$model$age))
...
newdata2 <- expand.grid(sex = levels(fit_titanic$model$sex),
                        passengerClass = levels(fit_titanic$model$passengerClass),
                        age = fit_titanic$model$age)
preds2 <- cbind(newdata2, survival = predict(fit_titanic, newdata = newdata2, type = "response"))
aggregate(survival ~ sex + passengerClass, data = preds2, FUN = mean)
```
...
newdata3 <- aggregate(age ~ sex + passengerClass, data = fit_titanic$model, FUN = identity)
...
aggregate(survival ~ sex + passengerClass, data = preds3, FUN = mean)
```
...
newdata1 <- newdata2 <- newdata3 <- fit_titanic_spaMM$data ## same as fit_titanic$model
...
- you cannot use `type = "response"` in `predict()` because transformations need to happen after CI are computed!
...
It occurs for any event when too many zeros occur.
