...
You can fit hurdle models with `glmmTMB::glmmTMB()`. glmmTMB is extremely flexible package that is still relativelly young but that look very promising.

```{r hurdle, message = FALSE}
library(glmmTMB)
fit_Zhurd1 <- glmmTMB(eggs ~ humans_eaten, ziformula = ~humans_eaten,
                      family = glmmTMB::truncated_poisson(link = "log"), data = AliensZ)
...
You can also fit zero-inflation models with `almmTMB::glmmTMB()`:
...
tab <- rbind(Poisson = c(fit_Zpoiss$coefficients, NA, NA),
             Hurdle = unlist(fixef(fit_Zhurd1)),  ## NOTE: the hurdle part predicts positive counts, not zeros!!
             ZeroInfl = unlist(fixef(fit_Zzi1)),  ## NOTE: the binary part predicts zeros, not counts!!
