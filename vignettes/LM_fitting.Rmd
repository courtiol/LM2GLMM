...
output:
  ioslides_presentation:
    ...
    smaller: true
vignette: >
  %\VignetteIndexEntry{2.1 Fitting procedure}
...
## Variance of sample and population

```{r sample var}
set.seed(1)
bias_sample_var <- replicate(100000, {
  observations <- rnorm(n = 20, mean = 0, sd = sqrt(5))
  5 - sum((observations - mean(observations))^2) / 20
})
mean(bias_sample_var)
```

```{r pop var}
set.seed(1)
bias_pop_var <- replicate(100000, {
  observations <- rnorm(n = 20, mean = 0, sd = sqrt(5))
  5 - sum((observations - mean(observations))^2) / 19
})
mean(bias_pop_var)
```

This is why we have learned (a long time ago for some of us) that the denominator of the population variance estimate should be $n - p$ to correct for the bias when you estimate p parameters (here $p = 1$, the parameter is the mean). 
...
## The R package {spaMM} can also fit LM

**{spaMM}** is an R package developed by François Rousset (a population geneticist).
...
- (and a little also because I work with François)
...
Note: in ML fit (default setting) `phi` contains the sample residual variance, but the error variance estimate can be directly obtained from a REML fit (more on that later). `lm()` does not have this flexibility, which become particularly useful for more complex models that aim at estimating variances (mixed models).
...
## Extraction of the sample residual variance
...
In the case of LM (and not GLM, LMM, GLMM), this is equivalent to performing the estimation
...
We can test that the function works by comparing the RSS for parameters set using `lm()`:
...
Alien$sample_residual_var <- sum(Alien$residuals^2) / nrow(Alien)
Alien$density <- dnorm(x = Alien$size, mean = Alien$fitted_values, sd = sqrt(Alien$sample_residual_var))
Alien[1, ]
```
...
curve(dnorm(x, mean = Alien[1, "fitted_values"],
            sd = sqrt(Alien$sample_residual_var[1])),
...
We can test that the function works by comparing the logLik for parameters set using `lm()`:
```{r alien log density}
...
compute_logLik <- function(vector.param, formula, data) {
  ...
  fitted_values <- X %*% matrix(vector.param) ## compute the fitted values given the coef
  response <- model.response(useful.data) ## extract the response
  sample_residual_var <- sum((response - fitted_values)^2) / nrow(useful.data)  ## compute the sample residual var
  sum(dnorm(response, mean = fitted_values, sd = sqrt(sample_residual_var), log = TRUE))
}
```
...
time2 <- system.time(
  fit_test_homemade_RSS <- optim(par = rep(0, 8),
                                 fn = compute_rss,
...
```{r computation time results}
round(rbind(c(coef(fit_test_lm), time1[1]),
            c(fit_test_homemade_RSS$par, time2[1]),
            c(fit_test_homemade_ML$par, time3[1])), 2)
```

<br>

So our methods work, but the are hugely inefficient!
...
* Note 1: $Q^\text{T}Y$ is also used to compute sum of squares efficiently in e.g. ```anova()```.
<!--Factor sum of squares is given by the first elements of QTY, it seems to depend on the design matrix. E.g. sum(QTY[1:2]^2) or just sum(QTY[2]^2). The, the residual sum of square would be given by sum(QTY[-c(1:2)]^2)```-->
* Note 2: other decompositions are sometimes used in other packages or other type of linear models (e.g. Cholesky).


## Actual `lm()` fitting procedure

First, `lm()` processes information for `lm.fit()`. Here are the important steps:
