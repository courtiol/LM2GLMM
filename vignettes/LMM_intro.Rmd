...
data.for.pred <- expand.grid(nitro = 0.3, Variety = "Victory", Block = levels(oatsyield$Block))
...
```
Note: unlike in **{spaMM}**, you have to do it by hand here...
...
```{r}
data.for.pred.with.b <- expand.grid(nitro = 0.3, Variety = "Victory", Block = levels(oatsyield$Block))
...
```{r}
fit_lmm_lme4_nitro <-  lmer(yield ~ nitro + Variety + (1|Block), data = oatsyield, REML = FALSE)
...
Note: always use ML fits for testing fixed effects (`anova()` from **{lme4}** does that automatically)
...
abline(0, 1, col = 2, lwd = 2, lty = 2)
```
Note: even the help file tells you that `aov()` is *"not particularly sensible statistically"*...
...
Note: here you must use a ML fit (not automatic!). The method relies on the so-called Satterthwaite's degrees of freedom method.
...
As for predictions, you can compute a marginal or a conditional AIC depending on whether you are interested in the fit under a random level or the observed ones in particular.
...
```{r, fig.width = 4, fig.height = 4}
#plot(fit_lmm_spaMM)  ## broken at the moment...
```
...
# Specifying multiple random predictors
...
```{r}
(fit_carnivora_rand_slope <- fitme(log_brain ~ log_body + (log_body|Family) + (1|Genus),
...
```{r}
ranef(fit_carnivora_rand_slope)$`( log_body | Family )`
```
...
data_pred <- predict(fit_carnivora_rand_slope,
...
## Testing if slopes differ between families using **{lme4}**

Asymptotic LRT are really poor for that, so better use parametric bootstrap:

```{r, warning=FALSE}
fit_rand_slope    <- lmer(log_brain ~ log_body + (log_body|Family) + (1|Genus), data = carnivora)
fit_no_rand_slope <- lmer(log_brain ~ log_body + (1|Family) + (1|Genus), data = carnivora)
pbkrtest::PBmodcomp(fit_rand_slope, fit_no_rand_slope)
```
...
## Testing if slopes differ between families using **{spaMM}**

```{r test slope spaMM boot, results="hide"}
fit_carnivora_rand_no_slope <- fitme(log_brain ~ log_body + (1|Family) + (1|Genus),
                                     data = carnivora, method = "REML")
LRT_obs <- -2 * (logLik(fit_carnivora_rand_no_slope) - logLik(fit_carnivora_rand_slope))  ## LRT observed
...
LRT_H0 <- spaMM_boot(fit_carnivora_rand_slope, simuland = LRT_slope, type = "marginal",
