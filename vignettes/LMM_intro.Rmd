...
(init_values <- c(bad_mod$coefficients[1], bad_mod$coefficients[2],
                 var(bad_mod$coefficients[-c(1:2)]), var(bad_mod$residuals)))
```
...
rbind(my.ranef, ranef_lme4 = as.numeric(t(ranef(mod_lme4)[[1]])))
```

* These numbers are often called the Best Linear Unbiased Predictions (BLUPs) of the random effects.
* Even if BLUPs can be estimated, they are not parameters of the statistical model!!!
* BLUPs should perhaps be computed after a REML and not ML fit... but as BLUPs are often combined with predicted responses which should be fitted using ML I am not so sure.
...
ranef(mod_lmm_lme4)  ## b estimates
```
...
```{r, fig.width = 4, fig.height = 4}
lattice::qqmath(ranef(mod_lmm_lme4, condVar = TRUE))
```
...
# Slightly more complex structure for random effects
...
### You can check the BLUPs structure to make sure you did it right!
...
```{r CI lambda lme4}
mod_lmer <- lmer(individual.speed ~ 1 + (1|group.id), data = d, REML = TRUE)
...
```{r}
plot(BLUPs ~ scale(mod2$data$BWT), ylab = "Predicted breeding values", xlab = "Scaled phenotypic values", las = 1)
```
...
(mod_spaMM <- fitme(RR ~ factor(alloc) + year + ablat + (1|study), data = dat.bcg, method ="REML",
...
mod_spaMMRE <- fitme(RR ~ 1 + (1|study), data = dat.bcg, method ="REML",
                    fixed = list(phi = dat.bcg$sampling.var))
...
get_meta_metrics(mod_spaMM, dat.bcg$sampling.var) ## add vi.avg: mean within study sampling variance
```
