...
library(merTools)
predictInterval(fit_lmm_lme4, newdata = data.for.pred.with.b, include.resid.var = FALSE)
```
Notes:
...
- `merTools::predictInterval()` uses simulations to build CI, so results differ slightly each time you run the function.
...
With **{spaMM}** you can still choose between ML and REML for GLMMs (REML sensu stricto does not exist for GLMMs for **{spaMM}** has implemented generalisation of the concept proposed in the statistical literature).
...
There is no full-fetched parametric bootstrap methods implemented as far as I know, nor conditional AIC, but this could change in the near future.
...
```
```{r fw_AIC_compared5}
AIC(fit_fw_glmmTMB_zipois, fit_fw_glmmTMB_zinb)
```
