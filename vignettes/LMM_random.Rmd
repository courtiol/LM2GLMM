...
* Do not use REML to perform LRT!
...
```{r}
mod2_ML <- fitme(y ~ x + (1|group), data = Aliens, method = "ML")
mod2_H0_ML <- fitme(y ~ x, data = Aliens, method = "ML")
pchisq(2*(logLik(mod2_ML) - logLik(mod2_H0_ML)), df = 1, lower.tail = FALSE)
mod2_H2_ML <- fitme(y ~ x + (1|group), data = Aliens, method = "ML", fixed = list(lambda = 2))
pchisq(2*(logLik(mod2_ML) - logLik(mod2_H2_ML)), df = 1, lower.tail = FALSE)
```
...
```{r}
mod_ML <- lmer(y ~ x + (1|group), data = Aliens, REML = FALSE)
mod_H0 <- lm(y ~ x, data = Aliens)
pchisq(2*(logLik(mod_ML) - logLik(mod_H0)), df = 1, lower.tail = FALSE)
anova(mod_ML, mod_H0)
```
...
set.seed(1L)
(LRTobs <- 2*(logLik(mod_ML)[[1]] - logLik(mod_H0)[[1]]))
LRTH0 <- replicate(200, {
                    Aliens$y <- simulate(mod_H0)[, 1]
                    2*(logLik(lmer(y ~ x + (1|group), data = Aliens, REML = FALSE))[[1]] -
...
set.seed(1L)
(LRTobs <- 2*(logLik(mod2_ML)[[1]] - logLik(mod2_H0_ML)[[1]]))
LRTH0_bis <- replicate(200, {
                    Aliens$y <- simulate(mod2_H0_ML)
                    2*(logLik(fitme(y ~ x + (1|group), data = Aliens, method = "ML"))[[1]] -
                      logLik(fitme(y ~ x, data = Aliens, method = "ML"))[[1]])
                    })
(sum(LRTH0 >= LRTobs) + 1) / (length(LRTH0) + 1)
```
...
```{r spaMM and boot, message = FALSE}
library(boot)
n_boot <- 1000
newYs <- simulate(mod2_ML, type = "residual", nsim = n_boot)  ## in progress, not sure how to simulate...
res_sim <- sapply(1:n_boot, function(i) {
  Aliens$y <- newYs[, i]
  fitme(y ~ x + (1|group), data = Aliens, method = "REML")$lambda[[1]]}) ## lambda on new data
boot.ci(boot.out = list(R = n_boot),
        t0 = fitme(y ~ x + (1|group), data = Aliens, method = "REML")$lambda[[1]], ## original lambda
        t  = matrix(res_sim, ncol = 1),
...
### ```spaMM```
```{r}
mod3noRS <- fitme(log_brain ~ log_body + (1|Family) + (1|Genus), data = carnivora)
