...
### Coverage N = 15
```{r  coverage boot 2 15, message = FALSE, warning = FALSE}
set.seed(1L)
test_poiss <- replicate(1000, {
  new.data <- simulate_Aliens_GLM(N = 15)
  mod <- update(mod_poiss, data = new.data)
  confint.myboot2(mod)
  })
...
Note: here, in this implementation, the deviance difference is divided by the dispersion (not LRT sensu stricto)...
...
## Comparing different links within the same family

Since the number of estimates do not change, you can use:
...
Since the response variable does not change, you can use:

* AIC
* deviance if the number of parameters is the same (dispersion counts!)
* LRT if the number of parameters varies (but don't use ```anova()``` to do it)
* parametric bootstrap
...
```{r, error = TRUE}
mod_gauss3 <- glm(eggs  ~ humans_eaten, family = gaussian(),  data = Aliens2)
LR <- -2 * (logLik(mod_gauss3)[[1]] - logLik(mod_poiss2)[[1]])
(pvalue <- 1 - pchisq(LR, 1))
anova(mod_gauss3, mod_poiss2, test = "LR")  ## does not count df well
mod_gauss3_spaMM  <- fitme(eggs  ~ humans_eaten, family = gaussian(),  data = Aliens2)
anova(mod_gauss3_spaMM, mod_poiss2_spaMM)  ## does not see the difference in model structure
```
