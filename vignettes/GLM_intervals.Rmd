...
Ok this is not very good either...
...
```{r}
set.seed(1L)
Aliens2 <- simulate_Aliens_GLM(N = 10)
...
The 3 tests (F, Wald and Likelihood ration test) give here the same statistics despite different p-values
```{r}
Anova(mod_gauss2, test = "Wald")
Anova(mod_gauss2, test = "LR")
```

## Anova: Gaussian

### The tests also don't differ with the summary results
```{r}
...
LRT(mod_gauss2_spaMM, mod_gauss2_spaMM0)  ## the value makes more sense than Anova
LRT(mod_poiss2_spaMM, mod_poiss2_spaMM0)
Anova(mod_gauss2_spaMM, mod_gauss2_spaMM0, boot.repl = 100)
```
