...
Tip: test by simulation using ```gaussian(link = "log")``` and ```poisson(link = "identity")```.
...
```{r pred int poiss}
pred <- predict(mod_poiss, newdata = newdata, se.fit = TRUE)
lwr <- pred$fit + qnorm(0.025) * sqrt(pred$se.fit^2 + poisson()$variance(poisson()$linkinv(pred$fit)))
upr <- pred$fit + qnorm(0.975) * sqrt(pred$se.fit^2 + poisson()$variance(poisson()$linkinv(pred$fit)))
poisson()$linkinv(cbind(lwr, upr))  ## don't forget the inverse link!
...
colors <- rep(NA, ncol(pv))
colors[pv["LR", ] > 0.05 & pv["z", ] > 0.05] <- "black"
colors[pv["LR", ] < 0.05 & pv["z", ] < 0.05] <- "green"
colors[pv["LR", ] > 0.05 & pv["z", ] < 0.05] <- "blue"
colors[pv["LR", ] < 0.05 & pv["z", ] > 0.05] <- "purple"
plot(pv["LR", ] ~ pv["z", ], xlim = c(0, 0.1), ylim = c(0, 0.1), col = colors)
abline(0, 1, col = "red", lty = 2, lwd = 2)
abline(v = 0.05, lty = 3)
abline(h = 0.05, lty = 3)
```
...
colors <- rep(NA, ncol(pv))
colors[pv["LR", ] > 0.05 & pv["F", ] > 0.05] <- "black"
colors[pv["LR", ] < 0.05 & pv["F", ] < 0.05] <- "green"
colors[pv["LR", ] > 0.05 & pv["F", ] < 0.05] <- "blue"
colors[pv["LR", ] < 0.05 & pv["F", ] > 0.05] <- "purple"
plot(pv["LR", ] ~ pv["F", ], xlim = c(0, 0.1), ylim = c(0, 0.1), col = colors)
abline(0, 1, col = "red", lty = 2, lwd = 2)
abline(v = 0.05, lty = 3)
abline(h = 0.05, lty = 3)
```
...
```{r comp 3, warning = FALSE}
set.seed(1L)
N <- 20
pv <- replicate(200, {
  d <<- data.frame(eggs = rpois(N, lambda = 2),
                  humans_eaten = round(runif(N, min = 0, max = 50)))
  mod_poiss_1      <- glm(eggs ~ humans_eaten, family = poisson(), data = d)
  mod_poiss_1_null <- glm(eggs ~ 1, family = poisson(), data = d)
  ...
  boot <- anova(mod_poiss_1_spaMM, mod_poiss_1_null_spaMM, boot.repl = 199, nb_cores = 4L)
...
par(las = 1)
plot(ecdf(pv["z", ]), xlim = c(0, 0.1), ylim = c(0, 0.1), col = "blue", cex = 0.5, main = "")
plot(ecdf(pv["LR", ]), xlim = c(0, 0.1), ylim = c(0, 0.1), col = "green", add = TRUE, cex = 0.5)
plot(ecdf(pv["Bartlett", ]), xlim = c(0, 0.1), ylim = c(0, 0.1), col = "orange", add = TRUE, cex = 0.5)
plot(ecdf(pv["Raw", ]), xlim = c(0, 0.1), ylim = c(0, 0.1), col = "purple", add = TRUE, cex = 0.5)
legend("topleft", fill = c("blue", "green", "orange", "purple"), legend = c("z", "LR", "Bartlett", "Raw"), bty = "n")
...
Note: you cannot compare deviances between ```gaussian()``` and other families unless you use the scaled deviance for the ```gaussian()```.
...
logLik_diff_obs <- logLik(mod_lm) - logLik(mod_glm)
