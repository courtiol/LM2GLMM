...
## Sample and population variance `r .emo("info")`
...
## Sample and population variance `r .emo("proof")`
...
## Extraction of the error variance estimate `r .emo("practice")`
...
- in **{spaMM}**, for ML fits (the default method) `phi` contains the population residual variance; i.e. denominator = $n$, not $n-p$
- in LM, the error variance estimate can be obtained as shown here, or it can be directly obtained from a REML fit (more on that later)
...
## Extraction of the population residual variance `r .emo("practice")`

### From fit with `stats::lm`()
```{r fit estimate residual var}
summary(fit_alien_lm)$sigma^2 * 
  (nrow(model.matrix(fit_alien_spaMM)) - ncol(model.matrix(fit_alien_spaMM))) / nrow(Alien)
```
...
## Home made LM fitting by OLS `r .emo("info")` `r .emo("proof")`
...
## Home made LM fitting by ML `r .emo("info")` `r .emo("proof")`
...
```
Let's compare parameter and log-likelihood values obtained to those obtained with `lm()`:
...
## Comparing the 3 methods on a REAL dataset `r .emo("info")`
...
## What the statistical textbooks tell us `r .emo("info")`
...
## Dissecting the output from `lm()` `r .emo("info")`
