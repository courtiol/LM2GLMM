...
mean(stat.rand >= stat.obs) #or better: (sum(stat.rand >= stat.obs) + 1) / (length(stat.rand) + 1)
```
...
LRT(mod_UK_milk_spaMM, mod_UK_no_milk_spaMM, boot.repl = 1000)
```
...
set.seed(1L)
(LRTobs <- 2*(logLik(mod_UK1) - logLik(mod_UK5)))
LRTH0 <- replicate(1000, {
                    UK_for_both$newY <- simulate(mod_UK5)[, 1]
                    2*(logLik(update(mod_UK1, newY ~. ))[[1]] -
...
refit_mod <- function(y, data, mod_null, mod_full, ...) {
  ...
  2*(logLik(mod_full_under_null) - logLik(mod_null))
}
LRTUK5null_boot <- spaMM_boot(mod_UK5_spaMM, simuland = refit_mod, nsim = 100,
