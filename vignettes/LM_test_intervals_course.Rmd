...
Note: REML fit are needed for whenever we are using variance estimates!
...
Note: to obtain the t-values, you need to rely on the `REML` fits because the denominator of the t-value (`Cond. SE`) are sqrt variance estimates.
...
```{r testing variances}
simu <- function(n, residvar) simulate_Aliens(N = n, sigma2 = residvar)

tests_var_under_H0 <- function(fit, n = nrow(fit$data), residvar_true = 25) {
  d <- simu(n, residvar_true)
...
- `ecdf()` computes the empirical cumulative distribution
- if a distribution is uniform, its ECDF is a straight line
- here, both methods look good!


# Confidence intervals for the residual variance


## Distribution of residual variance estimates `r .emo("nerd")`

Variance estimates generally follow a Gamma ($\Gamma$) distribution with mean  = $\texttt{estimate}$ and var = $2 (\texttt{estimate}^2) / \texttt{df}$.
...
CI_residvar_gamma <- function(fit) {
  ...
  qgamma(p = c(0.025, 0.975), shape = fit$df.residual/2, scale = 2*residvar/fit$df.residual)
}
```
```{r assymptotic 2}
...
Yet, for the particular case of the residual variance, OLS theory tells us to rely on the chi-square distribution (exact rather than asymptotic):
...
set.seed(123)
mean(replicate(1000,
  sum(findInterval(CI_residvar_chi2(update(fit_lm_AL1, data = simulate_Aliens())), 25)) == 1))  ## for lm fit
```
...
It is also possible to use a likelihood ratio test (LRT) as we did it before for testing $\widehat{\sigma}^2$. Instead of doing it by hand, we will use this time the simple function from the package **{lmtest}**:
...
- (it is equal to `sqrt(diag(X %*% vcov(fit) %*% t(X)))`)
...
Note: `se.pred` accounts for both the uncertainty in regression parameters and the uncertainty in the estimate for the residual variance.
