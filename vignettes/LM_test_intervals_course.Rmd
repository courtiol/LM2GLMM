...
* how to compute CI for predictions
...
**{spaMM}** does not (yet) compute the p-values for the t-tests but it is easy to compute them:
```{r summary AL1 2}
...
Note: to obtain the t-values, you need to rely on the `REML` fits because the denominator of the t-value is the estimate of the square root of the error variance.


## t-tests and linear models `r .emo("info")` `r .emo("proof")`

You can actually substitute simple t-tests with LM:
...
Note: the p-value obtained is the same `r .emo("party")`, but notice that effect sizes are opposite from each others.
...
Note: `update()` refit a model with some of its input updated, but you can instead recall `lm()` if you provide all its arguments. 
...
Note: `qt()` is used to extract the quantiles from the Student's t distribution and `sqrt(diag(vcov(fit_lm_AL1))` is used to unscale the result so it follows the scale of the response.
...
## Joint confidence intervals by likelihood profiling `r .emo("info")`

It can also be easily extended to build confidence regions (i.e. a multivariate CI):

```{r profile betas, echo = FALSE, results = "hide", fig.width = 4, fig.asp = 1}
...
```{r param boot spaMM}
confint(fit_spaMM_AL1_ML, parm = "humans_eaten", boot_args = list(nsim = 100))
```
...
## Point estimate for the error variance `r .emo("practice")`
...
Note: with **{spaMM}** you need to use the REML fit if you don't want to do maths.


## Testing differences in variance `r .emo("practice")`
...
For testing error variance in LM specifically, there is a better method:
...
```{r testing variances plot, echo=TRUE, fig.width=3.5}
...
# Confidence intervals of the error variance estimate


## Distribution of variance estimates `r .emo("nerd")`
...
$\texttt{shape} \times \texttt{scale}^2$ ($\texttt{shape}$ and $\texttt{scale}$ are the two parameters shaping the shape of the $\Gamma$ distribution); thus, little maths show that in our context $\texttt{shape} =  (n-p)/2$ and $\texttt{scale} = 2 \times\texttt{estimate}/(n-p)$.
...
## CI for $\widehat{\sigma}^2$ using the $\Gamma$ distribution `r .emo("info")`
...
## CI for $\widehat{\sigma}^2$ using the $\chi^2$ distribution `r .emo("practice")`
...
## Which method is best to build a CI? `r .emo("info")` `r .emo("proof")`
...
3- CI using likelihood profiling  `r .emo("slow")`
...
4- CI using parametric boostrap `r .emo("slow")` `r .emo("slow")` `r .emo("slow")`
...
Note: this example also shows that parametric bootstrap is not always good (something to keep in mind for the future).


# Tests on several regression coefficients at once


## Why testing several estimates at once? `r .emo("info")`
...
## Comparing models with the function `anova()` `r .emo("practice")`
...
## Comparing models with the function `anova()` `r .emo("info")`
...
((RSS_fit1 - RSS_fit2)/(fit_lm_UK1$df.residual - fit_lm_UK2$df.residual)) / summary(fit_lm_UK1)$sigma^2
```

Note: RSS/Res.Df = estimate of the error variance


## Comparing models with the function `lmtest::lrtest()` `r .emo("practice")`
...
## Comparing models with **{spaMM}** `r .emo("practice")`
...
**{spaMM}** makes it also easy to test the LRT by parametric bootstrap instead of using the $\chi^2$, which is good if we are far from asymptotic conditions (but it slow to run) `r .emo("slow")`:
...
- `anova(..., test = "LRT")` exists for model fitted with `lm()` but it does not really do a proper LRT `r .emo("broken")`


## Comparing models to test qualitative var. `r .emo("practice")`
...
## Comparing models to test quantitative var. `r .emo("practice")`
...
## Comparing models to test all predictors `r .emo("practice")`
...
`Anova()` performs a so-called Type 2 ANOVA: it tests for the effect of simple terms considering other simple terms to be modelled, but not interactions. The function also tests for the effect of interactions assuming all simple terms to be modelled.
...
## What shall I do? `r .emo("info")` `r .emo("proof")`
...
pred$fit
se.pred <- sqrt(pred$se.fit^2 + pred$residual.scale^2) ## residual.scale^2 = estimate of the error variance
