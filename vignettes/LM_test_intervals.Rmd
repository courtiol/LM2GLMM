...
output:
  ioslides_presentation:
    ...
    smaller: true
vignette: >
  %\VignetteIndexEntry{2.2 Tests & Intervals}
...
Note: `lm()` output are a little strange: the residual variance they provide is REML but the logLik is the one obtained by ML.


## Aliens fitted with `spaMM::fitme()`
...
Note: the ML fit is needed for whenever we are using the likelihood to study regression coefficients.
...
Note: the REML fit is needed for whenever we are using the estimate of the error.


## Brits fitted with `stats::lm()`
...
Point estimates are random variable. Thus, to know if these numbers differ from population parameter values, their distributions must be characterised and accounted for during tests.


## Testing regression coefficients with **{stats}**

Regression coefficient estimates are tested using Student's t-test:
...
Note: the null hypothesis for these tests is that the estimate is null.


## Testing regression coefficients with **{spaMM}**

Regression coefficient estimates are tested using Student's t-test:

```{r summary AL1}
(ttable <- summary(fit_spaMM_AL1_REML, verbose = FALSE)$beta_table)  ## verbose is just for the slides
```

**{spaMM}** does not compute the p-values but it is easy to compute them:
```{r summary AL1 2}
cbind(ttable, p = 2*pt(abs(ttable[, "t-value"]),
                       df = nrow(model.matrix(fit_spaMM_AL1_REML)) -
                         ncol(model.matrix(fit_spaMM_AL1_REML)),  ## n - p
                       lower.tail = FALSE))
```

Note: to obtain the t-values, you need to rely on the `REML` fits because the denominator of the t-test is the estimate of the square root of the error variance. Even in REML fits, however, the regression coefficients are estimated by ML as they should.


## t-tests and linear models

You can actually substitute simple t-tests by LM:
...
Note: the p-value obtained is the same!
...
```{r estimate normal, fig.width=4, fig.asp=1, fig.align='center'}
set.seed(123)
...
## Variances on estimates

Each regression parameter is associated with an estimation variance and since they are all being joinly estimated, there are also covariances between them:
...
There are sometimes refered to as *conditional* standard errors because these SE do not account for uncertainty stemming from other sources (which happens in LMM & GLMM).


## Testing a null but non-nill hypothesis

The t-test compare each regression coefficient to 0, but you may want to test something more specific.

For this we first need to understand how to do the previous t-tests by hand:
...
Then, you can simply change the numbers.
...
You can alternatively use the package **{car}** to do that more directly:
...
## Testing all null non-nill hypotheses at once: CI
...
- based on the student distribution (exact, but for LM only)
- by likelihood profiling (asymptotic, general)
- by parametric bootstrap (empirical, general)

<br>

In the case of LM the method based on the Student is best but we will see the alternative methods because they will become useful for more complex models and it is easier to start seeing them in the context of simple models.


# Confidence intervals on regression coefficients

## Student's based confidence intervals

This is the method available when using `confint()` on a `lm()` fit:
...
cbind(coef(fit_lm_AL1) + qt(0.025, 10) * sqrt(diag(vcov(fit_lm_AL1))),
      coef(fit_lm_AL1) + qt(0.975, 10) * sqrt(diag(vcov(fit_lm_AL1))))
```

Note: `qt()` is used to extract the quantiles from the Student's t distribution.


## Why are there other methods to compute CI?

A confidence interval with given level `X` is considered exact if the probability that the interval contains the value $k$ is `X` for any parameter value $k$. We call this probability *coverage*.
...
Depending on the type of linear model, some methods show a coverage closer to their nominal level than others.


## Measuring the coverage of Student's based CI
...
The coverage of our CI based on the Student's t distribution is great (this method is exact for LM, but not for LMM, GLM and GLMM).

Note: `update()` refit a model with some of its input updated, but you can instead recall `lm()`. 
...
```{r profile for slope, fig.align = "center", fig.width = 4, fig.height = 4, fig.show = "hold"}
...
abline(v = coef(fit_lm_AL1)["humans_eaten"], col = "blue", lwd = 2)
abline(h = logLik(fit_lm_AL1)[[1]] - 0.5*qchisq(0.95, df = 1), col = "red", lty = 2, lwd = 2)  ## the threshold
```
...
* Benefit: not based on SE, so works well even if error is not Gaussian (as in GLM).

<br>

* Cost: uses the chi-squared distribution to establish a threshold, which is an asymptotic approximation, so it needs a quite lot of observations for this approximation to be reasonable.


## Joint confidence intervals by likelihood profiling
...
In the specific case of the parametric bootstrap, we do that by simulating new data using the fitted model (i.e. considering the parameter estimates as population parameters) and refitting models based on such simulated data.
...
It is again a bit complicated even if the package **{boot}** helps:
...
CI_boot <- function(fit, param, nboot) {
  ...
  res_sim <- sapply(1:nboot, function(i) {
    ...
    coef(newfit)[[param]]  ## extract the estimate
  })
...
CI_boot(fit = fit_lm_AL1, param = "humans_eaten", nboot = 100)
```
Notes: for more complex models, there are packages doing that for you, but I do not know one that would work with LM (it may exist). Also, to use the function above with a **{spaMM}** fit, just run `coef.HLfit <- fixef.HLfit` beforehand.
...
## Comparing the 3 methods of CI construction
...
CONCLUSION: results are quite different depending on the method one uses!

In case of LM, it is a no brainer, stick to `confint()` which uses the Student's t distribution.


# Testing the error variance estimate

## Point estimate for the error variance

Remember how to extract the error variance:

```{r sigma point estimate}
(sigma2_error <- summary(fit_lm_AL1)$sigma^2)
fit_spaMM_AL1_REML$phi
```

<br>

Note: with **{spaMM}** you need to use the REML fit!


## Testing differences in variance

Testing if the error variance is null makes no sense, but it could make sense to compare the estimate to a given value.
...
But for testing error variance in LM specifically, there is a better method:

```{r sigma chisq}
pchisq(sum(residuals(fit_spaMM_AL1_REML)^2)/25, df = 10, lower.tail = FALSE)  ## 10 = n - p
```


## Which alternative to choose for testing?

Sometimes theory helps, but sometimes there is no clear answer. Again, you can use simulations to answer the question.
...
```{r testing variances}
simu <- function(n, sigma2) simulate_Aliens(n = n, sigma2 = sigma2)

tests_var_under_H0 <- function(fit, sigma2_true = 25, n = 12) {
  d <- simu(n, sigma2_true)
  fit_phi_est <- update(fit, data = d)
  fit_phi_fixed <- update(fit, fixed = list(phi = sigma2_true), data = d)
  stats_LRT <- -2 * (logLik(fit_phi_fixed) - logLik(fit_phi_est))
  c(chisq = pchisq(sum(residuals(fit_phi_est)^2)/sigma2_true, df = n - length(fixef(fit)), lower.tail = FALSE),
...
```{r testing variances plot, echo=TRUE, fig.align='center', fig.asp=1, fig.width=3.5}
...
Note: `ecdf()` computes the empirical cumulative distribution.

CONCLUSION: here, both methods look good!


# Confidence intervals on the error variance estimate


## Distribution of variance estimates

Variance estimates generally follow a Gamma ($\Gamma$) distribution with mean  = $\texttt{estimate}$ and var = $2 ( \texttt{estimate})^2 / (n-p)$.
...
curve(dgamma(x, shape = fit_lm_AL1$df.residual/2, scale = 2*sigma2_error/fit_lm_AL1$df.residual),
    from = 0, to = 100, xlab = "sigma2_error", ylab = "probability density", lwd = 3)
```


## CI for $\widehat{\sigma}^2$ using the $\Gamma$ distribution

So, we can rely on the Gamma to build a CI95% as follows:

```{r assymptotic}
confint_sigma2 <- function(fit) {
  sigma2_error <- summary(fit)$sigma^2
  qgamma(p = c(0.025, 0.975), shape = fit$df.residual/2, scale = 2*sigma2_error/fit$df.residual)
}
```
```{r assymptotic 2}
confint_sigma2(fit_lm_AL1)
```


## CI for $\widehat{\sigma}^2$ using the $\chi^2$ distribution

Yet, for the particular case of the estimate of the error variance, OLS theory tells us to use the chi-square distribution (exact rather than asymptotic):

```{r chi2 interval}
confint_sigma2_chi2 <- function(fit) {
 sum(residuals(fit)^2)/c(qchisq(0.975, df = fit$df.residual), qchisq(0.025, df = fit$df.residual))
}
```
```{r chi2 interval 2}
confint_sigma2_chi2(fit_lm_AL1)
```

<br>

If we did not know better, we could also use likelihood profiling or parametric bootstrap as we have seen it in the context of CI for regression coefficients.

Let's try that anyway for learning a thing or two...



## CI for $\widehat{\sigma}^2$ using likelihood profiling

```{r profile sigma, fig.align = "center", fig.width = 4, fig.height = 4}
candidate_sigma2 <- seq(10, 120, 5)
logLik_profile <- sapply(candidate_sigma2, function(sigma2) ## for each sigma2 we re-estimate other parameters
  logLik(update(fit_spaMM_AL1_REML_varfixed, fixed = list(phi = sigma2))))
plot(logLik_profile ~ candidate_sigma2, type = "o")
abline(v = 25, col = "green", lwd = 2)
abline(v = fit_spaMM_AL1_REML$phi, col = "blue", lwd = 2)
abline(h = logLik(fit_spaMM_AL1_REML) - 0.5*qchisq(0.95, 1), col = "red", lty = 2, lwd = 2)  ## threshold
```
...
```{r CI by profile var}
diff_logLik_sigma2 <- function(sigma2, fit) {
  newfit <- update(fit, fixed = list(phi = sigma2))
  logLik_newfit <- logLik(newfit)[[1]]
  logLik_goal <- logLik(fit)[[1]] - 0.5*qchisq(0.95, df = 1)
  abs(logLik_newfit - logLik_goal)
}
...
CI_prof_sigma2 <- function(fit, range = c(0.1, 1000)) {
  lwr <- optimise(diff_logLik_sigma2, interval = c(range[1], fit$phi),
                  fit = fit)
  upr <- optimise(diff_logLik_sigma2, interval = c(fit$phi, range[2]),
                  fit = fit)
  c(lwr = lwr$minimum, upr = upr$minimum)
}
```
...
CI_prof_sigma2(fit = fit_spaMM_AL1_REML)
```


## CI for $\widehat{\sigma}^2$ using parametric bootstrap

Again, you can use the package **{boot}** to perform a parametric bootstrap:

```{r CI param boot sigma2}
CI_boot_sigma2 <- function(fit, nboot = 1000) {
  newYs <- simulate(fit, nsim = nboot) ## simulate new responses from the fit!!
  res_sim <- sapply(1:nboot, function(i) {
    newdata <- model.frame(fit) ## extract the data
    newdata[, 1] <- newYs[, i] ## replace the response var
...
```{r CI param boot sigma2 2}
set.seed(123)
CI_boot_sigma2(fit = fit_spaMM_AL1_REML)
```

Note: that we obtained a negative value is not great since variances cannot be negative...


## Which method is best to build a CI?

Here we know: the one using the $\chi^2$ distribution, but sometime it is unclear (either because you don't know, because it is not known, or because there is no general answer) what the best method is. 

In such cases, you can always test for yourself by investigating the coverage of different methods by simulating data close to yours (as we did it earlier for Student's based CI).

Here, we will check the coverage for all 4 methods we have just mentioned:
...
set.seed(123)
mean(replicate(1000,
  sum(findInterval(confint_sigma2(update(fit_lm_AL1, data = simulate_Aliens())), 25)) == 1))  ## for lm fit
```

2- CI using the $\Gamma$

```{r coverage Gamma chi2}
set.seed(123)
mean(replicate(1000,
  sum(findInterval(confint_sigma2_chi2(update(fit_lm_AL1, data = simulate_Aliens())), 25)) == 1))  ## for lm fit
```


## Which method is best to build CI for $\widehat{\sigma}^2$?

3- CI using likelihood profiling

```{r coverage Gamma 2, eval = FALSE}
set.seed(123)
mean(replicate(1000,  ## SLOW!
  sum(findInterval(CI_prof_sigma2(update(fit_spaMM_AL1_REML, data = simulate_Aliens())), 25)) == 1))
## only designed to work with spaMM REML fit!
## RESULT  = 0.921
```

4- CI using parametric boostrap

```{r coverage Gamma 3, eval = FALSE}
set.seed(123)
mean(replicate(1000, ## VERY SLOW!
  sum(findInterval(CI_boot_sigma2(update(fit_spaMM_AL1_REML, data = simulate_Aliens()), nboot = 1000), 25)) == 1))  ## only designed to work with spaMM REML fit!
## RESULT  = 0.811
```

<br>

CONCLUSION: for LM, we can confirm that the method based on the $\chi^2$ is clearly the best one!

This example also shows that parametric bootstrap is not always good (something to keep in mind for the future).


# Tests on several regression coefficients at once


## Why testing several estimates at once?
...
- does the alcohol consumption during pregnancy have an effect of the height of children?
...
## Comparing models with the function `anova()`
...
The default behaviour for `anova()` on LM fit is to compare perform a F-test:
...
The F-test is used to check is the decrease in residual variance for the more complex fit is higher than expected under H0:

```{r F test hand}
RSS_fit1 <- sum(residuals(fit_lm_UK1)^2)
RSS_fit2 <- sum(residuals(fit_lm_UK2)^2)
((RSS_fit2 - RSS_fit1))/(fit_lm_UK2$df.residual - fit_lm_UK1$df.residual) / summary(fit_lm_UK1)$sigma^2
```

Note: RSS/Res.Df = estimate of the error variance


## Comparing models with the function `lmtest::lrtest()`

It is also possible to use a likelihood ratio test (LRT) as we did it before for testing $\widehat{\sigma}^2$. Here instead of doing it by hand, we will use this time the simple function from the package **{lmtest}**:

```{r lrtest LRT}
lmtest::lrtest(fit_lm_UK2, fit_lm_UK1)
```

Note: under LM the LRT and the F-test give similar results! (Not for GLM...)



## Comparing models with **{spaMM}**

It is possible to use `anova()` to perform a LRT if the model is fitted with **{spaMM}** (`lmtest::lrtest()` won't work):
...
**{spaMM}** makes it also easy to test the LRT by parametric bootstrap instead of using the $\chi^2$, which is good if we are far from asymptotic conditions (but it slow to run):

```{r spaMM LRT boot, eval = FALSE}
...
- `anova(..., test = "LRT")` exists for mode fitted with `lm()` but it does not really do a proper LRT (strange!)


## Comparing models to test a quantitative variable
...
## Comparing models to test all predictors
...
`Anova()` performs a so-called Type 2 ANOVA: it tests for the effect of simple terms considering other simple terms to be modelled, but not interactions. The it tests for the effect of interactions assuming all simple terms to be modelled.


## Comparing models to test all predictors

Do not use `anova()` for that: it performs the comparisons sequentially (i.e. Type 1 ANOVA, which is quite silly)...
...
## Comparing models to test against the null model

A null model is a model estimating nothing more than the mean of the response:
...
Note: **this test comparing a fit to the fit of a so-called null model should always be performed first before testing the effect of predictors individually!**
...
To study what happens when simulating many things, we will simulate predictors with no effect:
...
## Comparing many estimates increases false $+$
...
Some scientists (with poor knowledge in statistics) rely on so-called backward selection to discard predictors. This is a terrible thing if you want to do inference (i.e. test things).
...
* Always compare the fit to null model fit and do not do more tests if this global test is not significant
...
# Comparing factor levels using post-hoc analyses

## Comparing different levels from a factor
...
```{r ghlt spaMM, fig.align='center', fig.height = 3.5, fig.width = 10}
...
summary(multcomp::glht(fit_lm_UK1bis, linfct = multcomp::mcp(drink2 = c("Most_days - Not_at_all = 0",
                                                                        "Most_days = 0"))))
```
...
Note: although this is an established practice, it does not correct for multiple testing. 
...
$AIC = -2 \times (logLik - p)$, with $p$ the number of parameters
...
Since confidence intervals can be computed for any single-valued population parameter, you can compute confidence intervals around the mean response too!

The CI95% on the mean response is a nice way to represent the uncertainty of your fit:
...
## CI for the mean response with **{stats}**
...
Here is however a more general recipe that we will reuse for GLMs:
```{r compute predVar 2}
...
Note: `se.fit` only accounts for the uncertainty in the regression parameters. It can be obtained as `sqrt(diag(X %*% vcov(fit) %*% t(X)))`.


## CI for the mean response with **{spaMM}**
...
pred$fit <- predict(fit_spaMM_AL1_REML, newdata = pred)
pred$se.fit <- get_predVar(fit_spaMM_AL1_REML, newdata = pred)^0.5
rbind(c(pred$fit[1] + qt(0.025, 10) * pred$se.fit[1],
        pred$fit[1] + qt(0.975, 10) * pred$se.fit[1]),
      c(pred$fit[2] + qt(0.025, 10) * pred$se.fit[2], 
        pred$fit[2] + qt(0.975, 10) * pred$se.fit[2]))
```

Note: you must use REML fits!
...
It is used to predict the limits within which X% of new observations should fall (on average for future samples).

Example of PI95%:

```{r respVar, fig.align="center", fig.width=4.5, fig.height=4.5, echo = FALSE}
pred <- data.frame(humans_eaten = seq(1, 12, 0.1))
...
## Prediction intervals with **{stats}**
...
Note: `se.pred` accounts for both the uncertainty in regression parameters and the uncertainty in the estimate for the error variance.
...
pred <- data.frame(humans_eaten = 4:5)
pred$fit <- predict(fit_spaMM_AL1_REML, newdata = pred)
se.pred <- get_respVar(fit_spaMM_AL1_REML, newdata = pred, var = "respVar")^0.5
rbind(c(pred[1, "fit"] + qt(0.025, df = 10) * se.pred[1],
        pred[1, "fit"] + qt(0.975, df = 10) * se.pred[1]),
      c(pred[2, "fit"] + qt(0.025, df = 10) * se.pred[2],
        pred[2, "fit"] + qt(0.975, df = 10) * se.pred[2]))
```
