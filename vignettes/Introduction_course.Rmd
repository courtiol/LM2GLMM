...
```{r negative_index}
foo2[-3] #Remove one item from the vector
foo2[c(-3, -1)] #Remove multiple items
```
...
Logical operators compare two objects and return TRUE or FALSE (or NA)

```{r logical_operator}
10 == 11 #Is equal to
10 != 11 #Is not equal to
10 < 11  #Less than
...
foo  <- c(1, 2, 3)
foo <= 2 #Compare every item in vector to the same value
```

```{r logical_operator_vct2}
foo2 <- c(0, 3, 2)
foo < foo2 #Compare each set of numbers in the two vectors
```

```{r logical_operator_vct2}
foo > 0 & foo < 3 #Combine multiple operators 
```
...
mean(foo3 == "b") #Proportion of items that equal b
```
