...
## A desing matrix with interactions
...
round(M2b %*% c(-22.33333, -50.19960, -12.22020), 3)
```

## The case of polynomials: conversions

```{r convert poly}
convert_raw_to_ortho <- function(var, coef_raw) {
  ## the coef must contain the intercept!
  degree <- length(coef_raw) - 1
  X_ortho <- model.matrix(~ poly(var, degree))
  X_raw   <- model.matrix(~ poly(var, degree, raw = TRUE))
  solve(coef(lm.fit(X_raw, X_ortho)), coef_raw)
}

convert_ortho_to_raw <- function(var, coef_ortho) {
  ## the coef must contain the intercept!
  degree <- length(coef_ortho) - 1
  X_ortho <- model.matrix(~ poly(var, degree))
  X_raw   <- model.matrix(~ poly(var, degree, raw = TRUE))
  solve(coef(lm.fit(X_ortho, X_raw)), coef_ortho)
}

(coef_ortho <- convert_raw_to_ortho(var = somedata$x1, coef_raw = c(1, 2, -2)))
## (coef_raw <- convert_ortho_to_raw(var = somedata$x1, coef_ortho = coef_ortho)) ## recovers the input!
```
...
## The case of contrasts: conversion
