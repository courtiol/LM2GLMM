...
* how to understand the design matrix
* that the meaning of the parameter depends on the design matrix
* that the design matrix depends on the formula, the data and the contrats
...
Note that this design matrix is ill defined as some columns are linearly dependent.
...
This parameterization of factors such as ```x3``` follows the so-called contrast matrix "treatment".

It is the easiest one to interpret and the one use by default in R (but not in all software).

## Interpreting the treatment contrast

```{r treatment contrast}
model.matrix(object = ~ x3, data = somedata)
```

* The intercept represents the mean of the baseline (here, reference level = ```"a"```).
* ```x3b``` represents the mean of ```"b"``` minus the mean of the baseline.
* ```x3c``` represents the mean of ```"c"``` minus the mean of the baseline.

Note: in R the default level of reference is always the first level of a factor (which level is first can be changed).
...
Alternative parameterizations of the contrast matrix can ease convergence in GLM(M) (for continuous variable) and can allow for testing specific hypotheses or fit particular experimental design (for qualitative variables). 
...
It produces a symetric diagonal matrix. Those are particularly math friendly.

## Why make things more complex?

### For technical reasons

```{r helmert contrast 1}
...
The predictors in Helmert contrasts are also orthogonal. It also produces a symetric diagonal matrix.

## Why make things more complex?

### To ease the interpretation

```{r helmert contrast 2, echo = FALSE}
m
```
...
* The intercept thus represents the mean of the mean of all groups. (nice!)
* ```x1``` represents the mean of the first group minus the mean of the mean of all groups. (nice!)
* ```x2``` represents the mean of the second group minus the mean of the mean of all groups. (nice!)

Note: not much used by R users (because not the default) but seems more useful when there is no clear control.
...
Note: this works only with the contrast matrix "treatment".
...
### The meaning of the parameters always depends on the design matrix!!!

<br>

Consequence: if what the design matrix looks like is unclear for the reader, you must provide information (e.g. contrasts used, number of levels...).
...
## The example of treatment vs Helmert
...
If the average IQ for boys were 90 and 110 for girls, what would the parameter values be, for each coding?
