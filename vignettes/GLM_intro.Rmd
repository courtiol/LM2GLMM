...
This contains the predicted values for the observations at the original scale:
...
The link function converts the values from the original scale to the scale of the linear predictor.
...
The inverse link function converts the values of the linear predictor back into the original scale.
...
It should handle all families but it does not consider offsets, convergence issues and maybe other subtle things.
...
GLM are fitted using an iterative algorithm based on weighted least squares to update parameter estimates.

The GLM weights (not the prior weights) are inversely proportional to the variance of the response, but the variance depends on the mean which, in turn, depends on the parameters.

The algorithm is thus called IRLS for iteratively reweighted least squares, it is also sometimes called Fisher's scoring as it has been proposed by Fisher (in a context more general than GLM).

The IRLS algorithm fixes the weights, determines the parameter values that minimize the weighted sum of squared residuals, then updates the weights and repeats the process until the weights stabilize (it usually happens in very few iterations).

<br>

(I have paraphrased here *Douglas Bates*)
...
Parameter estimates can differ substantially:
...
Note: we compare the sum of squares (paying attention to do so on the same scale) as the models have been fitted on different responses which prevents the use of usual functions.
...
## The counter-example of the GLM Gaussian
...
The GLM2 log transforms the expected values, not the observations!

If SD small the methods converge!


# Interpreting parameter estimates

## Estimates and the Gaussian family
...
## Estimates and the Poisson family
...
```{r}
p.eta <- predict(mod_poiss, newdata = data.frame(humans_eaten = 1:10))
p.eta[-10] - p.eta[-1]
p.mu <- predict(mod_poiss, newdata = data.frame(humans_eaten = 1:10), type = "response")
p.mu[-10] - p.mu[-1]
```
...
## Estimates and the Binomial family
