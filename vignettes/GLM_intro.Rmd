...
spaMM::spaMM.options(nb_cores = 3L)
```
...
The objects of class ```family``` created by these functions also contain other useful information needed for the fitting procedure.
...
The true parameter values are stored as attributes if you need them"
```{r attributes}
...
```{r pv, message=FALSE}
fit_lm    <- lm(size  ~ humans_eaten, data = Aliens)
...
## The fitted GLM from `spaMM::fitme()`
...
We will see the 2 most useful components.
...
When things get complex, to convert the values from the scale of the response to the one of the linear predictor, use `linkfun()` from the family obejct instead of trying to do it by hand.
...
When things get complex, to convert the values from the scale of the linear predictor to the one of the response, use `linkinv()` from the family obejct instead of trying to do it by hand.
...
This algorithm is thus called IRLS for iteratively reweighted least squares, it is also related to sometimes called Fisher's scoring which has been proposed by Fisher (in a context more general than GLM).
...
The algorithm often quickly converges:
...
Check other options for the iterative procedure in `?glm.control`


## A toy mockup of `glm.fit()` (for geeks)
...
library(spaMM) ## you need to load spaMM explicitely for that
...
## Note: current spaMM has a bug, but devel and older versions work!!
```
...
```{r, error = TRUE}
fit_lm <- lm(log(eggs) ~ humans_eaten, data = Aliens)
```
...
```{r}
fit_lm2 <- lm(log(eggs+1) ~ humans_eaten, data = Aliens)
rbind(fit_poiss$coefficients,
...
round(rbind(
  fit_lm = c("mean int" = mean(simu[1, ]), "sd int" = sd(simu[1, ]),
             "mean slope" = mean(simu[2, ]), "sd slope" = sd(simu[2, ])),
  fit_glm = c(mean(simu[3, ]), sd(simu[3, ]), mean(simu[4, ]), sd(simu[4, ]))), 3)
```

In `fit_glm` the expected values are log transformed, not the observations! So the estimates produced by this fit are biased.
...
Here `fit_glm` corresponds the simulation model, so the estimates produced are better than the alternatives.
...
```{r}
coef(fit_poiss)
predict(fit_poiss, newdata = data.frame(humans_eaten = 10))
...
```{r}
(p.eta <- predict(fit_poiss, newdata = data.frame(humans_eaten = 1:5)))
...
Eating one additional human increases the average number of eggs produced by aliens by ```r exp(fit_poiss$coefficients[2])``` times or ```r 100*exp(fit_poiss$coefficients[2])-100```%.

Note: if you change the link function, the receipe will be different!


## Estimates and the Binomial with logit link

```{r}
(p.eta <- predict(fit_binom, newdata = data.frame(humans_eaten = 1:5)))
...
Eating one additional human increases the **odd** of aliens having blue eyes by ```r exp(fit_binom$coefficients[2])``` times.
...
When the dispersion parameter is not estimated, `summary` uses $z$, not $t$. Yet, these tests are not very reliable and it is better to rely on CI build using likelihood profiling or paramettric bootstrap.
...
The F-test performs very badly here, but the LRT is better.


## Anova by LRT:

```{r Anova Poiss}
anova(fit_poiss, update(fit_poiss, . ~ 1), test = "LRT")
...
## Comparing test types using **{spaMM}**
...
plot(ecdf(pv_boot["raw", ]), col = "purple", add = TRUE, cex = 0.5)
legend("topleft", fill = c("blue", "green", "orange", "purple"), legend = c("F", "LR", "Param boot Bartlett", "Param boot raw"), bty = "n")
...
## Which methods should you use to compute CI?
...
If **{MASS}** is installed calling `confint()` on a model fitted with `stats::glm()` direcly does build CI by likelihood profiling:
...
For model fitted with `spaMM::fitme()`, calling `confint()` also perform likelihood profiling:

```{r Lev bug}
LevenbergM <- list() ## to avoid a bug in current spaMM... (fixed in devel)
```
...
For model fitted with `spaMM::fitme()`, calling `confint()` can also be used to compute CI using parametric boostrap if the argument `boot_args` is correctly set:
...
Which method is best depends on the particular circumstances, but you can test for yourself using simulations:
...
## CI using Gaussian approximation

There is no longer a direct method using `predict()` for models fitted with `stats::glm()`:
```{r conf fitted poiss}
...
cbind(lwr = poisson(link = "log")$linkinv(lwr_eta),
      upr = poisson(link = "log")$linkinv(upr_eta))
```
Note: you cannot use `type = "response"` in `predict()` because transformations need to happen after CI are computed!
...
get_intervals(fit_poiss_spaMM_ML, newdata = newdata, intervals = "predVar")  ## log to get eta!
```
...
Note: using the deviance is not recommended because some family return a scaled deviance while other do not...
...
# Residuals (and interlude before checking assumptions)
...
## Many types of residuals can be computed for GLM
...
Unfortunately those residuals are prety much useless to check the model assumptions!
...
## Computing residuals by simulation: a better idea
...
plot(ecdf_simulated_1 <- ecdf(simulated_data_cont[1, ]), main = "cdf of first fitted value")
...
for (i in 1:nrow(fit_poiss$model)) {
  ecdf_fn <- ecdf(simulated_data_cont[i, ])
...
## Computing residuals by simulation with **{DHARMa}**
...
plot(s_bad <- simulateResiduals(fit_poiss_bad, n = 1000))
```
...
plot(s_good <- simulateResiduals(fit_poiss_good, n = 1000))
```
...
car::crPlots(fit_poiss_bad, terms = ~ humans_eaten)
```
...
Asymptotic tests based on these measures do not always behave well:
...
testDispersion(r2)  ## you can change options to test for underdispersion
```
...
It kind of works, but we cannot do much with these types of fit:
```{r confint quasipoiss}
confint(fit_poissMixQ)
c(logLik(fit_poissMixQ), AIC(fit_poissMixQ))
```
...
## Modelling departure from expected dispersion with other probability distributions
...
c(AIC(fit_poissMix_spaMM), AIC(fit_poissMixNB_spaMM), logLik(fit_poissMixNB_spaMM))
```
...
p <- predict(fit_poissMix_spaMM, newdata = d, intervals = "predVar")
...
p <- predict(fit_poissMixNB_spaMM, newdata = d, intervals = "predVar")
...
p <- predict(fit_poissMixCP_spaMM, newdata = d, intervals = "predVar")
...
mean(sum(AliensZ$eggs == 0) / sum(dpois(0, fitted(fit_Zpoiss))))
```
...
fit_Zhurd1 <- hurdle(eggs ~ humans_eaten | humans_eaten, dist = "poisson", zero.dist = "binomial",
                     data = AliensZ)
fit_Zhurd2 <- hurdle(eggs ~ humans_eaten | 1, dist = "poisson", zero.dist = "binomial", data = AliensZ)
lmtest::lrtest(fit_Zhurd1, fit_Zhurd2)
mean(sum(AliensZ$eggs == 0) / sum(predict(fit_Zhurd1, type = "prob")[, 1]))
```
...
mean(sum(AliensZ$eggs == 0) / sum(predict(fit_Zzi1, type = "prob")[, 1]))
```
...
```{r comp zeroinfl}
cbind(Poisson = AIC(fit_Zpoiss),
      Hurdle  = AIC(fit_Zhurd1),
      ZeroInf = AIC(fit_Zzi1))
...
Note: you need an additional package but **{spaMM}** will inform you about it.
