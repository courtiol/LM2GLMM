...
testTemporalAutocorrelation(insect_glm_resid)
```
...
testTemporalAutocorrelation(esoph_probit_resid, time = mod_esoph_probit$model$agegp_f) ## useful??
testTemporalAutocorrelation(esoph_probit_resid, time = mod_esoph_probit$model$tobgp_f) ## useful??
testTemporalAutocorrelation(esoph_probit_resid, time = mod_esoph_probit$model$alcgp_f) ## useful??
...
testOverdispersion(resid_Titanic)
```

The overdispersion is significant but very small, so we will ignore it!
...
As the effect of age looks quite linear except for the category in which we have few data we also try a GAM
...
testOverdispersion(cauchit_gam_resid)
```
