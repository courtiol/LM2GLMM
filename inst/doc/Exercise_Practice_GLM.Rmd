...
The effect of have smooked is not significant (LRT, LR = ```r round(LR, 1)```, df = ```r round(DF, 2)```, p-value = ```r round(PV, 2)```).
...
We will now plot the predictions for the model with the logit link, the model with the cauchit link and the model with the GAM model with the logit link to explore their differences. We will predict the influence of age and consider  the median BMI. Also, the smoking satus has no significant effect so we will perform the prediction considering smoker since we have data for this category.
...
Selecting the best model, let's now look at the effect of BMI (we won't look at smoking since it does not significantly influence the outcome):
...
This gives the information for individuals for normal BMI. Let's noew look at the corresponding proportion for the obeses:
...
tab
```
That seems a good selection process (+/- 1 year and the whole WHO normal BMI category), so let's check the proportion of menopaused individuals in each of these categories:
