...
mod <- fitme(y ~ 0 + (1|group) + offset(50 + 1.5*x), data = Aliens, fixed = list(lambda = 2, phi = 0.5))
logLik(mod)
```
...
rbind(my.ranef, sapMM_ranef = ranef(mod)[[1]])
```
...
Perform the predictions using the function predict instead, both using ```spaMM``` and ```lme4```.


## Testing if slopes differ between fanilies
...
### Two between genus variances among families: 
...
```{r}
plot(BLUPs ~ mod2$data$BWT, ylab = "Predicted breeding values", xlab = "Phenotypic value", las = 1)
```
